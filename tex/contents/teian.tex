...
ファインチューニングを実行することで, QAデータセットを使用してモデルを学習させて, アプリの欠陥や要望を尋ねる質問に対して元のレビューからキーフレーズを抽出できるようにする. % キーフレーズはモデルの学習中における損失関数の計算に使用される. モデルはキーフレーズを抽出するように学習され, 損失が最小化されるようにパラメータが調整される. 
