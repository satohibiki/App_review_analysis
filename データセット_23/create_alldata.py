...
def create_tweet():
    ...
    for app_name in app_names:
        with open(f'../twitter/preprocessing_Twitter_data(23_10_01~23_12_20)/{app_name}.csv', 'r', encoding='utf-8') as csv_file:
...
def create_google():
    ...
    output = []
    for app_name in app_names:
        with open(f'../google_play/preprocessing_google_data(23_10_01~23_12_20)/{app_name}.csv', 'r', encoding='utf-8') as csv_file:
            csv_reader = csv.reader(csv_file)
            rows = list(csv_reader)
            random.shuffle(rows)
            for row in rows[1:]:
